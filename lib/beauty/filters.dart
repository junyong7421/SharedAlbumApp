...
import 'dart:ui' as ui;                  // ← show 빼고 전체 별칭
import 'dart:math' as math;              // ← 소문자 권장

// image 패키지는 하나만 써도 되지만, 너 코드 흐름 맞추려고 둘 다 유지 가능
import 'package:image/image.dart' as img;      // skin smoothing에서 사용
import 'package:image/image.dart' as img_pkg;  // 나머지에서 사용
...
Future<Uint8List> rasterizeMask(
  ...
  // 1) 배경을 완전 투명으로
  final bg = ui.Paint()..color = const ui.Color(0x00000000);
  canvas.drawRect(rect, bg);

  // 2) 마스크(흰색)를 feather 값에 따라 blur 적용해서 그림
  ...
  if (feather > 0) {
    // feather 픽셀을 대략적인 sigma로 사용 (원하면 *0.8 등으로 미세조정)
    paint.maskFilter =
        ui.MaskFilter.blur(ui.BlurStyle.normal, feather.toDouble());
  }
...
Future<Uint8List> enlargeEyes({
  ...
  required double amount, // 0~1 (0=변화없음, 1=강함)
  required int width,
  required int height,
}) async {
  if (amount <= 0) return bytes;

  final src = img_pkg.decodeImage(bytes);
  if (src == null) return bytes;

  final out = src.clone();

  // _warpCircle 함수 전체 삭제
...
img_pkg.Pixel _get(img_pkg.Image im, int x, int y) =>
...
void warpAt(ui.Offset c) {
  ...
  for (int y = (cy - r).floor(); y <= (cy + r).ceil(); y++) {
    ...
    for (int x = (cx - r).floor(); x <= (cx + r).ceil(); x++) {
      ...
      final dist = math.sqrt(dist2); // ← 소문자 math
  ...
  // dart:math import
  // 파일 상단에 추가:  import 'dart:math' as Math;

  warpAt(leftCenter);
  warpAt(rightCenter);

  return Uint8List.fromList(img_pkg.encodePng(out));
}
...
Future<Uint8List> tintLips({
  ...
  required int width,
  required int height,
  double satGain = 0.2,      // 채도 증폭량(0~1)
  double hueShiftDeg = 0.0,  // 색상 회전(deg)
  double intensity = 0.6,    // 결과 블렌딩 강도(0~1)
}) async {
  final src = img_pkg.decodeImage(bytes);
  if (src == null) return bytes;
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++, idx++) {
      final mask = lipMaskAlpha[idx] / 255.0; // 0~1
      if (mask <= 0) continue;
